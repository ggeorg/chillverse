...
/**
 * This class handles TXT record data for DNS based service discovery as
 * specified http://tools.ietf.org/html/draft-cheshire-dnsext-dns-sd-11
 * <p>
 * DNS-SD specifies that a TXT record corresponding to an SRV record consist of
 * a packed array of bytes, each preceded by a length byte. Each string is an
...
public class DnsSdTxtRecord {
  ...
  public DnsSdTxtRecord(byte[] data, int size) {
   ...
   * Returns null if index exceeds the total number of keys. Returns null if the
   * key is present with no value.
   */
